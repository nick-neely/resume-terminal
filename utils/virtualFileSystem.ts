          ...
          children: {
            ...
            contact: {
              ...
              children: Object.fromEntries(
                Object.entries(parsedResume.personalInfo.contact).map(([key, value]) => [
                  `${key}.txt`,
                  {
                    name: `${key}.txt`,
                    type: 'file',
...
function changeDirectory(vfs: VFS, path: string): VFS {
  ...
  for (const dir of newPath) {
    if (current.children && current.children[dir] && current.children[dir].type === 'directory') {
      current = current.children[dir];
    } else {
...
export { changeDirectory, getCurrentDirectory, initialVFS, listDirectory, readFile };

export type { VFS, VFSNode };
