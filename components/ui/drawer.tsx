...
>(({ className, children, ...props }, ref) => (
  <DrawerPortal>
    ...
    <DrawerPrimitive.Content
      ...
      {...props}
    >
      <div className="mx-auto mt-4 h-1 w-[60px] rounded-full bg-zinc-700/50" />
      {children}
    </DrawerPrimitive.Content>
  </DrawerPortal>
));
...
const DrawerHeader = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div
    className={cn('grid gap-1.5 p-4 text-center sm:text-left text-zinc-100', className)}
    {...props}
  />
);
...
const DrawerFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn('mt-auto flex flex-col gap-2 p-4 text-zinc-100', className)} {...props} />
);
